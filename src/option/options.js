...
import {
  ...
  availableLanguages,
  availableResponseLanguages,
} from "./languagesSettings.js";
  ...
  const setVoices = () => {
    ...
    const hasTarget =
      ...
      ttsVoiceSelect.querySelector(
        `option[value="${CSS.escape(targetVoiceName)}"]`
      );
    const hasPrev =
      previousValue &&
      ttsVoiceSelect.querySelector(
        `option[value="${CSS.escape(previousValue)}"]`
      );
...
function handleCustomModelSelection(
  ...
  const isCustom = ![...selectElement.options].some(
    (opt) => opt.value === savedModelValue && opt.value !== "custom"
  );
  if (isCustom) {
...
function saveOptions() {
  ...
  chrome.storage.local.get(Object.keys(keysToUpdate), (result) => {
    ...
    const finalData = {
      [settingsStorageKey]: {
        ...(result[settingsStorageKey] || {}),
        ...keysToUpdate[settingsStorageKey],
      },
      [promptsStorageKey]: finalPrompts,
      [voiceSettingsStorageKey]: {
        ...(result[voiceSettingsStorageKey] || {}),
        ...keysToUpdate[voiceSettingsStorageKey],
      },
    };
...
function loadOptions() {
  ...
  chrome.storage.local.get(
    [promptsStorageKey, settingsStorageKey, voiceSettingsStorageKey],
    (result) => {
      if (chrome.runtime.lastError)
      ...
      if (
        !savedVoiceSettings ||
        typeof savedVoiceSettings.sttLanguage === "undefined" ||
        typeof savedVoiceSettings.ttsVoiceName === "undefined"
      ) {
        ...
        chrome.storage.local.set(
          ...
          () => {
            if (!chrome.runtime.lastError) {
              if (elements.sttSelect)
                elements.sttSelect.value = currentVoiceSettings.sttLanguage;
              populateVoiceList();
            }
          }
        );
      ...
      if (elements.uiScaleValue)
        elements.uiScaleValue.textContent = `${uiScale}%`;
      document.documentElement.style.fontSize = `${uiScale}%`;

      const savedLocalMode =
        savedApiSettings.localApiMode ?? defaultApiSettings.localApiMode;
      const localRadio = q(
        `input[name="localApiModeSelection"][value="${savedLocalMode}"]`
      );
      if (localRadio) localRadio.checked = true;

      if (elements.vllmUrlInput)
        elements.vllmUrlInput.value =
          savedApiSettings.localApiUrl ?? defaultApiSettings.localApiUrl;

      handleCustomModelSelection(
        elements.ollamaModelSelect,
        elements.customOllamaModelContainer,
        elements.customOllamaModelInput,
        savedApiSettings.localMultimodalModel ??
          defaultApiSettings.localMultimodalModel
      );
      ...
      handleCustomModelSelection(
        ...
        savedApiSettings.lmstudioModelName ??
          defaultApiSettings.lmstudioModelName
      );

      if (elements.vllmModelInput)
        elements.vllmModelInput.value =
          savedApiSettings.vllmModelName ?? defaultApiSettings.vllmModelName;

      const savedCloudProvider =
        savedApiSettings.cloudProvider ?? defaultApiSettings.cloudProvider;
      const savedCloudMethod =
        savedApiSettings.cloudApiMethod ?? defaultApiSettings.cloudApiMethod;
      let cloudRadio;
      ...
      if (elements.cloudProxyUrlInput)
        elements.cloudProxyUrlInput.value =
          savedApiSettings.cloudProxyUrl ?? defaultApiSettings.cloudProxyUrl;
      if (elements.cloudApiKeyInput)
        elements.cloudApiKeyInput.value =
          savedApiSettings.cloudApiKey ?? defaultApiSettings.cloudApiKey;
      if (elements.gcpApiKeyInput)
        elements.gcpApiKeyInput.value =
          savedApiSettings.gcpApiKey ?? defaultApiSettings.gcpApiKey;
      if (elements.cloudModelNameInput)
        elements.cloudModelNameInput.value =
          savedApiSettings.cloudModelName ?? defaultApiSettings.cloudModelName;

      if (elements.mistralApiKeyInput)
        elements.mistralApiKeyInput.value =
          savedApiSettings.mistralApiKey ?? defaultApiSettings.mistralApiKey;
      if (elements.mistralModelSelect)
        elements.mistralModelSelect.value =
          savedApiSettings.mistralModelName ??
          defaultApiSettings.mistralModelName;

      if (elements.apimEndpointInput)
        elements.apimEndpointInput.value = savedApiSettings.apimEndpoint ?? "";
      if (elements.apimSubscriptionKeyInput)
        elements.apimSubscriptionKeyInput.value =
          savedApiSettings.apimSubscriptionKey ?? "";
      if (elements.azureFoundryModelNameInput)
        elements.azureFoundryModelNameInput.value =
          savedApiSettings.azureFoundryModelName ?? "";

      if (elements.roleSelect) {
        const activeKey =
          currentPrompts.active_system_prompt_key || "web_assistant";
        elements.roleSelect.value = activeKey;
      }

      if (elements.responseLanguageSelect) {
        elements.responseLanguageSelect.value =
          currentPrompts.responseLanguage || defaultPrompts.responseLanguage;
      }
...
function resetToDefaults() {
  chrome.storage.local.get([settingsStorageKey], (result) => {
    if (chrome.runtime.lastError) {
      showNotification(
        `Error reading settings: ${chrome.runtime.lastError.message}`,
        true
      );
      return;
    }
    ...
    chrome.storage.local.set({ [settingsStorageKey]: settings }, () => {
      if (chrome.runtime.lastError) {
        showNotification(
          `Error resetting settings: ${chrome.runtime.lastError.message}`,
          true
        );
      } else {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const elementsToSave = qa(
    "input[data-storage-key], select[data-storage-key]"
  );
  elementsToSave.forEach((el) => {
    const eventType =
      el.tagName === "SELECT" ||
  ...
  if (ollamaModelSelect && customOllamaContainer) {
    ollamaModelSelect.addEventListener("change", () => {
      customOllamaContainer.classList.toggle(
        "hidden",
  ...
  if (lmstudioModelSelect && customLmstudioContainer) {
    lmstudioModelSelect.addEventListener("change", () => {
      customLmstudioContainer.classList.toggle(
        "hidden",
        lmstudioModelSelect.value !== "custom"
      );
    });
  }
