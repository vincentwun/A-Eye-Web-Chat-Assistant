...
class AIScreenReader {
  ...
  _getProcessedSystemPrompt(systemPromptTemplate) {
    ...
    const languageName =
      availableResponseLanguages[responseLangCode] || "English (US)";
  ...
  _setupDependencies() {
    ...
    const proxiedApiService = {
      sendRequest: (config, history, prompt, image, systemPrompt) => {
        ...
        const finalSystemPrompt = this._getProcessedSystemPrompt(
          combinedSystemPromptTemplate
        );
        const genericPrompt = {
          prompt:
            "Analyze the provided information and follow the system prompt's immediate task.",
        };
        return originalApiService.sendRequest(
          config,
          history,
          genericPrompt,
          image,
    ...
    this.actionDependencies = {
      ...
      getHistoryToSend: this.stateManager.getHistoryToSend.bind(
        this.stateManager
      ),
  ...
  _setupActions() {
    ...
    this.contentAnalysisAction = new ContentAnalysisAction(
      this.actionDependencies
    );
    this.getElementAction = new GetElementAction(this.actionDependencies);
    this.actionFlowController = new ActionFlowController(
      this.actionDependencies
    );
    ...
    const commandProcessorActions = {
      ...
      _executeScrollingScreenshot: () =>
        this.screenshotAction.execute("scrolling"),
      _executeContentAnalysis: this.contentAnalysisAction.execute.bind(
        this.contentAnalysisAction
      ),
      handleError: this.handleError.bind(this),
    };
  ...
  async initializeAll() {
    try {
      ...
      this.uiManager.updateModeUI(
        currentState.activeApiMode ?? defaultApiSettings.activeApiMode
      );
    }
  }
  ...
  _canPerformAction({ checkConfig = true } = {}) {
    ...
    if (checkConfig) {
      ...
      if (
        state.activeApiMode === "local" &&
        !this.configValidator.isLocalModeConfigValid()
      )
        return false;
      if (
        state.activeApiMode === "cloud" &&
        !this.configValidator.isCloudModeConfigValid()
      )
        return false;
    }
    return true;
  ...
  _withActionGuards(
    handler,
    { ignoreProcessing = false, checkConfig = true } = {}
  ) {
    return async (event) => {
      if (!this.stateManager.isSettingsLoaded()) {
        this.appendMessage("system", "Initializing, please wait...");
  ...
  _setupEventListeners() {
    const handlers = {
      ...
      onScrollingScreenshot: this._withActionGuards(
      ...
      onToggleVoice: this._withActionGuards(
        this.voiceController.toggleVoiceInput.bind(this.voiceController),
        { checkConfig: false }
      ),
      onRepeat: this._withActionGuards(this.handleRepeat, {
        checkConfig: false,
      }),
      onModeChangeLocal: this._withActionGuards(
        () => this.handleModeChange("local"),
        { checkConfig: false }
      ),
      onModeChangeCloud: this._withActionGuards(
        () => this.handleModeChange("cloud"),
        { checkConfig: false }
      ),
      onClear: this._withActionGuards(this.handleClear, {
    ...
    chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
      const messageHandlers = {
        toggleApiMode: () =>
          this.handleModeChange(
            this.stateManager.getState().activeApiMode === "local"
              ? "cloud"
              : "local"
          ),
      ...
      if (handler) {
        const needsConfigCheck =
          request.type !== "toggleApiMode" &&
          request.type !== "toggleRepeat" &&
          request.type !== "toggleVoiceInput";
        if (!this._canPerformAction({ checkConfig: needsConfigCheck })) {
          sendResponse({
            success: false,
            error: "Cannot perform action right now",
          });
          return true;
        }
        try {
  ...
  async updateLastImageData(dataUrl, mimeType) {
    ...
    if (
      this.uiManager &&
      typeof this.uiManager.appendPreviewMessage === "function"
    ) {
  ...
  async handleSendMessage() {
    ...
    try {
      ...
      const activePromptKey =
        prompts.active_system_prompt_key || "web_assistant";
    ...
    try {
      ...
      if (commandResult) {
        if (commandResult.command === "executeActions") {
          await this.actionFlowController.handleExecuteActionsRequest(
      ...
      this.handleError(
        "Failed to process command from response",
        processorError
      );
      this.setProcessing(false);
    }
  ...
  async handleRepeat() {
    this.voiceController.stopSpeaking();
    const lastAIMessage = [...this.stateManager.getState().messages]
      .reverse()
