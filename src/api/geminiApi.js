export async function sendGeminiRequest(apiConfig, standardMessages, mimeType) {
  ...
  if (connectionMethod === "proxy") {
    ...
    if (!apiConfig.gcpApiKey)
      throw new Error("GCP API Key not configured for Vertex AI (GCP) method.");
  ...
  try {
    ...
    try {
      ...
      throw new Error(
        `API Error (${response.status} ${
          response.statusText
        }). Response: ${errorBodyText.substring(0, 200)}`
      );
    }
    ...
    if (!response.ok) {
      ...
      if (data && data.error) {
        if (typeof data.error === "string") {
          ...
          detailedError += `. Response: ${JSON.stringify(data.error).substring(
            0,
            200
          )}`;
        }
      } else if (data) {
        detailedError += `. Response: ${JSON.stringify(data).substring(
          0,
          200
        )}`;
      }
    ...
    let responseText = `Error: Could not parse ${
      connectionMethod === "proxy" ? "proxied" : ""
    } Gemini response.`;
    try {
      if (data.candidates && data.candidates[0]?.content?.parts) {
        ...
        if (data.promptFeedback.safetyRatings) {
          responseText += ` - Details: ${data.promptFeedback.safetyRatings
        ...
        const safetyRatingsInfo = data.candidates[0].safetyRatings
          ?.map((r) => `${r.category}: ${r.probability}`)
          .join(", ");
        if (safetyRatingsInfo)
          responseText += ` (Safety Ratings: ${safetyRatingsInfo})`;
        if (data.candidates[0].content?.parts?.some((p) => p.text)) {
          const partialText = data.candidates[0].content.parts
            .filter((p) => p.text)
            .map((p) => p.text)
            .join("");
      ...
      responseText = `Error: Failed to process ${
        connectionMethod === "proxy" ? "proxied " : ""
