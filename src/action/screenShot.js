export class ScreenshotController {
      ...
      chrome.runtime.sendMessage(
        ...
        (response) => {
          if (chrome.runtime.lastError) {
            console.error(
              "getPageInfo - Runtime Error:",
              chrome.runtime.lastError
            );
            reject(
              new Error(`Messaging error: ${chrome.runtime.lastError.message}`)
            );
            return;
          }
          if (!response) {
            console.error(
              "getPageInfo - No response received from background."
            );
            reject(
              new Error(
                "Failed to get page dimensions: No response from background."
              )
            );
            return;
          }
          if (response.error) {
            console.error(
              "getPageInfo - Error from background:",
              response.error
            );
            reject(
              new Error(`Failed to get page dimensions: ${response.error}`)
            );
            return;
          }
          if (
            response.dimensions === null ||
            response.dimensions === undefined
          ) {
            console.error(
              "getPageInfo - Dimensions are null or undefined in response:",
              response
            );
            reject(
              new Error(
                "Failed to get page dimensions: Background returned null/undefined dimensions."
              )
            );
            return;
          }
          console.log(
            "Received dimensions from background:",
            response.dimensions
          );
  ...
  async handleScrollingScreenshot(tab) {
    ...
    try {
      ...
      console.log(
        "handleScrollingScreenshot - Dimensions received:",
        pageDimensions
      );
      if (!this.validatePageInfo(pageDimensions)) {
        throw new Error(
          "Invalid page dimensions received (e.g., clientHeight is 0 or values are not numbers)."
        );
      }

      if (pageDimensions.scrollHeight <= pageDimensions.clientHeight + 1) {
        console.log(
          "Page doesn't need scrolling. Capturing single visible tab."
        );
        const singleShot = await this.captureVisibleTab();
        if (!singleShot) {
          throw new Error(
            "Failed to capture single visible tab for non-scrolling page."
          );
        }
        return singleShot;
      }
  ...
  async captureScreenshots(tab, pageInfo) {
    ...
    const overlap = 60;
    const stepSize =
      clientHeight > overlap ? clientHeight - overlap : clientHeight;

    console.log(
      `Starting scrolling capture: scrollHeight=${scrollHeight}, clientHeight=${clientHeight}, stepSize=${stepSize}, maxScroll=${maxScroll}`
    );
    ...
    while (this.scrollingScreenshotImages.length < MAX_CAPTURES) {
      ...
      if (capturedPositions.has(actualScrollY)) {
        console.log(
          `Already captured at scroll position ${actualScrollY}. Ending capture.`
        );
        break;
      }

      console.log(
        `Capturing at scroll: ${actualScrollY} (Attempt ${
          this.scrollingScreenshotImages.length + 1
        })`
      );
      ...
      try {
        ...
        if (screenshot) {
          this.scrollingScreenshotImages.push({
            image: screenshot,
            y: actualScrollY,
          });
          ...
          console.warn(
            `captureVisibleTab returned empty at scroll: ${actualScrollY}.`
          );
        }
      } catch (error) {
        if (
    ...
    console.log(
      `Scrolling capture finished. Captured ${this.scrollingScreenshotImages.length} images.`
    );
  }
  ...
  async executeScroll(tab, scrollPosition) {
    try {
      ...
      console.error(
        `Failed to execute scroll to ${scrollPosition} on tab ${tab.id}:`,
        error
      );
  ...
  async mergeScreenshots(capturedData, pageDimensions) {
    ...
    if (imgWidth === 0 || totalHeight <= 0) {
      console.error("Cannot merge: Invalid dimensions.", {
        imgWidth,
        totalHeight,
      });
      throw new Error(
        "Cannot merge screenshots: Invalid dimensions (width is 0 or scrollHeight is 0)."
      );
    }
    ...
    for (let i = 0; i < capturedData.length; i++) {
      ...
      try {
        const image =
          i === 0 ? firstImageLoaded : await this.loadImage(item.image);
        console.log(`Drawing image ${i} at y: ${item.y}`);
        ctx.drawImage(image, 0, item.y);
      } catch (loadError) {
        console.error(
          `Failed to load and draw screenshot index ${i} for merging:`,
          loadError
        );
      }
    }
