...
function findAllInteractableElements() {
  ...
  function generateSelector(element) {
    ...
    try {
      ...
      for (const attr of testAttrs) {
        ...
        if (value) {
          const testSelector = `${element.tagName.toLowerCase()}[${attr}="${CSS.escape(
            value
          )}"]`;
      ...
      if (nameAttr) {
        const nameSelector = `${element.tagName.toLowerCase()}[name="${CSS.escape(
          nameAttr
        )}"]`;
      ...
      if (nameAttr && classes) {
        const tagNameClassSelector = `${element.tagName.toLowerCase()}[name="${CSS.escape(
          nameAttr
        )}"]${classes}`;
        if (document.querySelectorAll(tagNameClassSelector).length === 1)
          return tagNameClassSelector;
      }
      return classes
  ...
  const selectors = [
    ...
    "[onclick]:not(body):not(html):not(div)",
  ...
  document.querySelectorAll(selectors.join(", ")).forEach((element) => {
    ...
    if (
      ...
      typeof elementInfo.text === "string"
    ) {
...
export async function findElementsOnPage() {
  try {
    ...
    const elements = await executeScriptOnTab(
      tab.id,
      findAllInteractableElements
    );
    ...
    return {
      success: false,
      error: `Error finding elements: ${error.message}`,
    };
  }
}
