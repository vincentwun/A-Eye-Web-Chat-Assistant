    ...
    try {
      try {
        ...
        if (
          ...
          (!injectionError.message.includes("already been injected") &&
            !injectionError.message.includes(
              "Cannot create multiple injection"
            ))
        ) {
          console.warn(
            "Readability injection may have failed:",
            injectionError
          );
        } else {
      ...
      const executionFunction = () => {
        if (typeof Readability === "undefined") {
          console.warn(
            "Readability library not found in content script, falling back to body.innerText"
          );
        ...
        try {
          ...
          if (!article || !article.textContent) {
            console.warn(
              "Readability could not parse meaningful content, falling back to body.innerText"
            );
      ...
      if (!results || !results[0] || !results[0].result) {
        if (results && results[0] && results[0].frameId === undefined) {
          console.error(
            "Content extraction script likely failed to execute properly (missing frameId). URL:",
            tab.url
          );
          throw new Error(
            "Content script did not execute correctly. Check page permissions or content script errors in the target page's console."
          );
        }
        console.error(
          "Content extraction script did not return a valid result object:",
          results
        );
        throw new Error(
      ...
      if (
        ...
        console.warn(
          `Content extraction unsuccessful or content is empty. Method: ${
            extractedData.method || "Unknown"
          }. Error: ${errorMsg}`
        );
        throw new Error(
          `Content extraction failed: ${errorMsg} (Method: ${
            extractedData.method || "N/A"
          })`
        );
      }

      console.log(
        `Content extraction successful. Method: ${
          extractedData.method
        }, Title: ${extractedData.title || "N/A"}, Length: ${
      ...
      const message =
        ...
        error.message.includes('tab URL: "chrome')
