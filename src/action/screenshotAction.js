export class ScreenshotAction {
  constructor(dependencies) {
    ...
    if (
      ...
      !this.setProcessing ||
      !this.appendMessage ||
      !this.updateLastImageData
    ) {
      console.error("ScreenshotAction missing dependencies:", dependencies);
      throw new Error(
        "ScreenshotAction initialized with missing dependencies."
      );
    }
  ...
  async execute(type = "visible") {
    ...
    try {
      ...
      if (type === "visible") {
        ...
        const mimeTypeMatch = imageDataUrl
          ? imageDataUrl.match(/^data:(image\/(?:jpeg|png|webp));base64,/)
          : null;
        ...
        if (!tab || !tab.id)
          throw new Error(
            "No active tab found or tab ID missing for scrolling screenshot."
          );
        imageDataUrl =
          await this.screenshotController.handleScrollingScreenshot(tab);
        mimeType = "image/png";
      ...
      if (imageDataUrl) {
        ...
        throw new Error(
          `${
            type === "scrolling" ? "Scrolling screenshot" : "Screenshot capture"
          } returned empty data.`
        );
      }
    } catch (error) {
      const errorMessage =
        type === "scrolling"
          ? "Scrolling screenshot analysis failed"
