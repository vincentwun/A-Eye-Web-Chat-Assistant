export class UIManager {
  constructor(voiceController) {
    ...
    if (
      ...
      !this.elements.localModeButton ||
      !this.elements.cloudModeButton
    ) {
      console.error(
        "UIManager initialized with missing essential elements (conversation or mode buttons)."
      );
    }
    if (!this.elements.currentModeIndicator) {
      console.warn(
        "UIManager initialized without currentModeIndicator element."
      );
    }
  }
  ...
  _getElements() {
    this.elements = {
      ...
      scrollingScreenshotButton: document.getElementById(
        "scrolling-screenshot-button"
      ),
      ...
      pastedImagePreviewContainer: document.getElementById(
        "pasted-image-preview-container"
      ),
      pastedImagePreview: document.getElementById("pasted-image-preview"),
      removePastedImageButton: document.getElementById(
        "remove-pasted-image-button"
      ),
    };
  }
  ...
  bindEventListeners(handlers) {
    const eventMap = {
      ...
      scrollingScreenshotButton: {
        event: "click",
        handler: handlers.onScrollingScreenshot,
      },
      analyzeContentButton: {
        event: "click",
        handler: handlers.onAnalyzeContent,
      },
      ...
      removePastedImageButton: {
        event: "click",
        handler: handlers.onRemovePastedImage,
      },
    ...
    for (const elementId in eventMap) {
      ...
      if (element) {
        const events = Array.isArray(eventMap[elementId])
          ? eventMap[elementId]
          : [eventMap[elementId]];
  ...
  appendMessage(role, formattedContent) {
    ...
    this.elements.conversation.scrollTop =
      this.elements.conversation.scrollHeight;
  }
  ...
  _enhanceCodeBlocks(container) {
    ...
    codeBlocks.forEach((codeBlock) => {
      ...
      langName.textContent = langClass
  ...
  async appendPreviewMessage(type, content) {
    ...
    previewContainerDiv.classList.add(
      "message",
      "message-user",
      "message-preview"
    );
  ...
  updateModeUI(activeMode) {
    ...
    if (!localBtn || !cloudBtn) {
      console.warn(
        "Cannot update mode UI, mode buttons not found in elements."
      );
      return;
    }
  ...
  updateInputState() {
    ...
    this.elements.sendButton.disabled =
      (!inputValue || inputValue.trim().length === 0) && !hasImage;
  }
  ...
  setProcessingState(isProcessing) {
    ...
    if (
      isProcessing &&
      this.elements.voiceButton.classList.contains("active-recording")
    ) {
  ...
  showPastedImagePreview(dataUrl) {
    if (
      !this.elements.pastedImagePreviewContainer ||
      !this.elements.pastedImagePreview
    )
      return;
